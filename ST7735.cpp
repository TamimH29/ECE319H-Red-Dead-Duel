...
// converter program.
// (x,y) is the screen location of the top left corner of BMP image
// Requires (11 + 2*w*h) bytes of transmission (assuming image fully on screen)
...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
    y = _height - y; // Changes orientation of coordinates to new orientation

    if(((x + h) > _width) || ((y - w + 1) > _height) || (x < 0) || (y < 0)){
    ...
    if((y - w) < 0 || (x + h) > _width || x < 0 || y > _height){            // image exceeds right of screen
        return;
    }

    setAddrWindow(x, y-w+1, x+h-1, y);
    //  setAddrWindow(0, 0, 20, 20);

    for(y=w - 1; y>=0; y--){
        for(x=0; x<h; x++){
            uint16_t color = ~(image[x*w + y]); // For Evan's Screen;
            //uint16_t color = ST7735_SwapColor(image[x*h + y]);
                                            // send the top 8 bits
            SPI_OutData((uint8_t)(color >> 8));
    ...
    for (j = 0; j<8; j++) {
      if (line & 0x1) {
        if (size == 1) // default size
          ST7735_DrawPixel(x+j, _height - (y+i), textColor);
        else {  // big size
          ST7735_FillRect(x+(j*size), _height - (y+(i*size)), size, size, textColor);
        }
      } else if (bgColor != textColor) {
        if (size == 1) // default size
          ST7735_DrawPixel(x+j, _height - (y+i), bgColor);
        else {  // big size
          ST7735_FillRect(x+(j*size), _height - (y+(i*size)), size, size, bgColor);
        }
      }
      line >>= 1;
    }
...
// outputs: none
void ST7735_SetCursor(uint32_t newY, uint32_t newX){
  if((newX > 12) || (newY > 26)){       // bad input
    return;                             // do nothing
  }
  StX = newX;
  StY = newY;
...
void ST7735_OutUDec(uint32_t n){
  ...
  StY = StY+Messageindex;
  if(StY>26){
    StY = 26;
    ST7735_DrawCharS(StX*10,StY*6,'*',ST7735_RED,ST7735_BLACK, 1);
  }
}
...
// Outputs: none
void ST7735_OutChar(char ch, uint8_t size, uint16_t color){
  if((ch == 10) || (ch == 13) || (ch == 27)){ // New line
    StX += size; StY=0;
    if(StX>15){
      StX = 0;
    }
  ...
  ST7735_DrawCharS(StX*10,StY*6,ch,color,ST7735_BLACK, size);
  if(StY>27 - size){
    StY = 27 - size;
    ST7735_DrawCharS(StX*10,StY*6,'*',ST7735_RED,ST7735_BLACK, size);
  }
  StY += size;
  return;
}
...
void ST7735_OutChar(char ch){
  ST7735_OutChar(ch, 1, StTextColor);
}
...
// outputs: none
void ST7735_OutString(char *ptr, uint8_t size, uint16_t color){
  while(*ptr){
    ST7735_OutChar(*ptr, size, color);
    ptr = ptr + 1;
  }
}

void ST7735_OutString(char *ptr){
  ST7735_OutString(ptr, 1, StTextColor);
}
// ************** ST7735_SetTextColor ************************
...
const char Hello_English[] =" Welcome to Red Dead Duel!";
const char Hello_Spanish[] =" \xAD" "Bienvenido a Duelo Rojo" "\n " "Muerto!";
const char Language_English[]=" English";
const char Language_Spanish[]=" Espa\xA4ol";
const char Sw_E[] =" Press Switch 1 (left) for" "\n " "English";
const char Sw_S[] =" Presiona el interruptor 2" "\n " "(derecha) para espa" "\xA4" "ol";
const char Mayhem_E[] =" You found a secret!" "\n " "Mayhem mode will" "\n " "begin...";
const char Mayhem_S[] =" Has encontrado un" "\n " "secreto! El modo" "\n " "de caos comenzara...";
const char Shot_E[] =" Great Shot!";
const char Shot_S[] =" \xAD" "Gran disparo!";
const char Gameover_English[]=" Game Over";
const char Gameover_Spanish[]=" Fin del juego";
const char P1_E[]=" Congratulations, John" "\n " "(left) has won!";
const char P1_S[]=" \xAD" "Felicidades, John\n " "(izquierda) ha ganado!";
const char P2_E[]=" Congratulations, Arthur" "\n " "(right) has won!";
const char P2_S[]=" \xAD" "FFelicidades, Arthur" "\n " "(derecha) ha ganado!";
const char Tie_E[] = " It's a draw." "\n" " You both died.";
const char Tie_S[] = " \xAD" "Es un empate." "\n" " Ambos murieron.";
const char End_E[]=" Hit any switch to" "\n " "play again!";
const char End_S[]=" \xADPresiona cualquier" "\n " "interruptor para jugar" "\n " "de nuevo!";
const char *Phrases[2][12]={
  {Hello_English, Hello_Spanish, Language_English, Sw_E, Sw_S, Mayhem_E, Shot_E, Tie_E, Gameover_English, P1_E, P2_E, End_E},
  {Hello_Spanish, Hello_English, Language_Spanish, Sw_E, Sw_S, Mayhem_S, Shot_S, Tie_S, Gameover_Spanish, P1_S, P2_S, End_S}
};

// Outputs a phrase
void OutPhrase(phrase_t message){
    ST7735_OutString((char *)Phrases[myLanguage][message]);
    ST7735_OutChar(13);
}
